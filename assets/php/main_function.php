...
==========================================================================*/
	...
	/*
		Edit or delete design function
	*/

	function editDesignContent($path, $content)
	{

		$file = fopen("design/$path/index.html", "w");
		fwrite($file, $content);
		fclose($file);
	}
	...
	*/
	...
	function displayDesignContent($designtab)
	{
			...
			<script>
				$(document).ready(function(){
					...
					$('.sidemenu').html(`
						...
						listDesignTab("$designtab");		// display idea side menu tab

		echo "
						</div>
					`);
					$('.topic').html('$designtab<a href=\'#addDesign\'><button  class=\'btn btn-default addIdeaBtn\' type=\'button\' value=\'$designtab\'>Add Design	</button></a> ');

					$('.documentationContentSection').show();

					//Display design content by load the html document
					$('.contentSection').load(encodeURI('design/$designtab/index.html'),function(){
					});
	 ...
	 /*
		Display design side menu tab function
	 */
	 function listDesignTab($currentTab)
	 {
		 $directories = glob('design/*' , GLOB_ONLYDIR);
		 $projectList="<div class='ideaTabListMenu'> <ul class='ideaList'> ";
		 echo $projectList;
		 foreach ($directories as &$projectName) {
			$projectName = explode("/", $projectName);
			if($currentTab == $projectName[1])
				$projectList="<a href='?designTab=$projectName[1]'><li  class='ideaListItem active' value='$projectName[1]' >$projectName[1] </li></a>";
			else
				$projectList="<a href='?designTab=$projectName[1]'><li class='ideaListItem' value='$projectName[1]' >$projectName[1] </li></a>";
			echo $projectList;
		 }
		 $projectList="</ul></div> ";
		 echo $projectList;
	 	...
	 	*/
	 	...
	 	function addDesignContent($path, $content)
	 	{

	 		//prepend
	 		$cache_new = "$content"; // this gets prepended
	 		$file = "design/$path/index.html"; // the file to which $cache_new gets prepended
